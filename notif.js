(function(){try{if(localStorage.getItem('bt-dark')==='1')document.documentElement.classList.add('bt-dark');}catch(e){}
  ...
  function injectStyle(){
    ...
    +'#bt-banner{position:fixed;left:50%;top:56px;transform:translateX(-50%);z-index:99997;max-width:90%;background:'+EMER+';color:#fff;padding:11px 15px;border-radius:12px;box-shadow:0 6px 20px rgba(0,0,0,.25);font:500 14px/1.4 -apple-system,Segoe UI,Roboto,Arial,sans-serif;cursor:pointer;text-align:center;display:none;}'+'html.bt-dark{--bg:radial-gradient(circle at 6% 2%,rgba(30,111,217,.30),transparent 40%),radial-gradient(circle at 96% 100%,rgba(34,211,238,.22),transparent 42%),#0d1420;--card:rgba(28,38,52,.82);--ink:#e7edf3;--muted:#9aa9b8;--faint:#6b7a8b;--line:#27313f;--pos-soft:rgba(30,111,217,.18);}#bt-dark{position:fixed;top:9px;right:max(58px,calc(50vw - 204px));z-index:99998;width:38px;height:38px;border-radius:50%;background:rgba(255,255,255,.16);display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,.2);line-height:1;}html.bt-dark #bt-dark{background:rgba(255,255,255,.10);color:#fff;}';
  ...
  function build(){ injectStyle(); if(document.getElementById('bt-bell'))return; var bell=document.createElement('div'); bell.id='bt-bell'; bell.innerHTML=BELL+'<span id=\"bt-badge\"></span>'; var panel=document.createElement('div'); panel.id='bt-panel'; panel.innerHTML='<h4>Notifications</h4><div id=\"bt-list\"></div>'; var banner=document.createElement('div'); banner.id='bt-banner'; banner.onclick=function(){banner.style.display='none';}; document.body.appendChild(bell); document.body.appendChild(panel); document.body.appendChild(banner); E.bell=bell; E.badge=document.getElementById('bt-badge'); E.panel=panel; E.list=document.getElementById('bt-list'); E.banner=banner; bell.onclick=function(ev){ev.stopPropagation();toggle();};var dk=document.createElement('div');dk.id='bt-dark';function dupd(){dk.textContent=document.documentElement.classList.contains('bt-dark')?'\u2600':'\u263E';dk.style.fontSize='18px';}dupd();dk.onclick=function(ev){ev.stopPropagation();var on=document.documentElement.classList.toggle('bt-dark');try{localStorage.setItem('bt-dark',on?'1':'0');}catch(e){}dupd();};document.body.appendChild(dk); document.addEventListener('click',function(e){ if(open && !panel.contains(e.target) && e.target!==bell && !bell.contains(e.target)){ open=false; panel.style.display='none'; } }); }
